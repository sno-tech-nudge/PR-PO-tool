  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--ink)' }}>Tally Export</div>
        ...
        <button
          ...
          style={{
            ...
            background: showConfig ? 'var(--ink)' : 'var(--surface-card)',
            color: showConfig ? 'var(--surface-card)' : 'var(--ink)',
            border: '1px solid var(--taupe-200)', fontSize: '12px', cursor: 'pointer',
          }}
        >
          Ledger Settings
        </button>
      ...
      {showConfig && (
        <div style={{
          ...
          padding: '18px', marginBottom: '20px', background: 'var(--taupe-50)',
        }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '8px' }}>
            {Object.keys(DEFAULT_LEDGER_MAP).map(cat => (
              <div key={cat} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{
                  fontSize: '11px', color: 'var(--ink)', width: '160px', flexShrink: 0,
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--action)', marginBottom: '4px' }}>
            How to import into TallyPrime
          </div>
          <div style={{ fontSize: '11px', color: 'var(--action)', lineHeight: 1.6 }}>
      ...
      {!loading && reports.length === 0 && (
        <div style={{ textAlign: 'center', padding: '48px 0' }}>
          <div style={{ fontSize: '32px', marginBottom: '12px', color: 'var(--taupe-400)', fontWeight: 300 }}>—</div>
          <div style={{ fontSize: '14px', fontWeight: 500, color: 'var(--ink)', marginBottom: '4px' }}>No reimbursed reports yet</div>
          <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Tally entries will appear here once reports are approved or reimbursed</div>
        </div>
      )}
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span
                onClick={toggleAll}
                style={{ fontSize: '12px', color: 'var(--ink)', cursor: 'pointer', textDecoration: 'underline' }}
              >
              ...
              {exported && (
                <span style={{ fontSize: '12px', color: 'var(--moss-text)', fontWeight: 500 }}>✓ Downloaded</span>
              )}
            </div>
          ...
          {reports.map(report => {
            ...
            return (
              <div key={report.id} style={{
                border: `1px solid ${isSelected ? 'var(--ink)' : 'var(--taupe-200)'}`,
                borderRadius: 'var(--radius-lg)', marginBottom: '8px', overflow: 'hidden',
                background: isSelected ? 'var(--taupe-50)' : 'var(--surface-card)',
              }}>
                <div style={{ display: 'flex' }}>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      borderRight: '1px solid var(--taupe-100)', cursor: 'pointer', flexShrink: 0,
                    }}
                  >
                    <div style={{
                      width: '16px', height: '16px',
                      border: `1.5px solid ${isSelected ? 'var(--ink)' : 'var(--taupe-400)'}`,
                      borderRadius: 'var(--radius-sm)', background: isSelected ? 'var(--ink)' : 'var(--surface-card)',
                  ...
                  <div style={{ flex: 1, padding: '12px 14px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4px' }}>
                      <div>
                        ...
                        <span style={{
                          ...
                          background: report.status === 'reimbursed' ? 'var(--moss-bg)' : 'var(--gold-bg)',
                          color: report.status === 'reimbursed' ? 'var(--moss-text)' : 'var(--gold-text)',
                        }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border: '1px solid var(--taupe-200)', fontSize: '11px', cursor: 'pointer',
                      }}
                    >
                ...
                {isPreviewed && previewEntries && (
                  <div style={{ borderTop: '1px solid var(--taupe-100)', background: 'var(--taupe-50)', padding: '14px' }}>
                    ...
                    <div style={{ overflowX: 'auto' }}>
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                        <thead>
                          <tr style={{ borderBottom: '1px solid var(--taupe-200)' }}>
                            {['Dr/Cr', 'Ledger Account', 'Category / Note', 'Amount'].map(h => (
                              <th key={h} style={{ textAlign: 'left', padding: '6px 10px', fontSize: '10px', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase' }}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {previewEntries.debits.map((d, i) => (
                            <tr key={i} style={{ borderBottom: '1px solid var(--taupe-100)' }}>
                              <td style={{ padding: '8px 10px' }}>
                                <span style={{ fontWeight: 600, color: 'var(--action)', background: 'var(--action-bg)', padding: '2px 6px', borderRadius: 'var(--radius-sm)', fontSize: '10px' }}>Dr</span>
                              </td>
                              <td style={{ padding: '8px 10px', color: 'var(--ink)', fontWeight: 500 }}>{d.ledger}</td>
                              <td style={{ padding: '8px 10px', color: 'var(--text-muted)' }}>{d.category}{d.vendors ? ` — ${d.vendors}` : ''}</td>
                              <td style={{ padding: '8px 10px', color: 'var(--ink)', fontWeight: 600, textAlign: 'right', fontFamily: 'monospace' }}>
                                ₹{Number(d.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                              </td>
                            </tr>
                          ))}
                          {previewEntries.credits.map((c, i) => (
                            <tr key={i} style={{ borderBottom: '1px solid var(--taupe-100)', background: '#FFF' }}>
                              <td style={{ padding: '8px 10px' }}>
                                <span style={{ fontWeight: 600, color: 'var(--clay-text)', background: 'var(--clay-bg)', padding: '2px 6px', borderRadius: 'var(--radius-sm)', fontSize: '10px' }}>Cr</span>
                              </td>
                              <td style={{ padding: '8px 10px', color: 'var(--ink)', fontWeight: 500 }}>{c.ledger}</td>
                              <td style={{ padding: '8px 10px', color: 'var(--text-muted)' }}>Reimbursement payment</td>
                              <td style={{ padding: '8px 10px', color: 'var(--ink)', fontWeight: 600, textAlign: 'right', fontFamily: 'monospace' }}>
                          ...
                          {/* Totals row */}
                          <tr style={{ borderTop: '2px solid var(--taupe-200)', background: 'var(--taupe-50)' }}>
                            <td colSpan={3} style={{ padding: '8px 10px', fontSize: '11px', fontWeight: 600, color: 'var(--ink)' }}>Total</td>
                            <td style={{ padding: '8px 10px', fontWeight: 700, textAlign: 'right', fontFamily: 'monospace', color: 'var(--ink)' }}>
...
const inputStyle = {
  width: '100%', height: '36px', border: '1px solid var(--taupe-200)', borderRadius: 'var(--radius-md)',
  padding: '0 10px', fontSize: '12px', color: 'var(--ink)', outline: 'none',
  background: 'var(--surface-card)', boxSizing: 'border-box', fontFamily: 'inherit',
}
